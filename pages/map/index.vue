               ...
               <div v-else>
                    <div>
                         ...
                         <ul class="grid grid-cols-1 divide-y bg-white rounded-md shadow-md" >
                              <li v-for="marker in mapPoints" :key="marker._id?.toString()" class="hover:bg-gray-50 p-2">
                                   <NuxtLink class="p-2 mb-2 w-full" :to="`/map/${marker._id}`">
                                        {{ marker.name }}
                                   </NuxtLink>
                              </li>
                         </ul>
                         <el-button type="primary" class="w-max mt-4 " @click="dialogVisible = true">
                              Add point
                              <el-icon class="ml-2">
                                   <ElIconPlus />
                              </el-icon>
